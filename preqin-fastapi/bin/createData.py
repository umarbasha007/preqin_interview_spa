...
def generate_postal_code(city):
    ...
    postal_code_formats = {
        ...
        "Singapore": "{} {}{}".format(
            *random.choices(string.digits, k=3), *random.choices(string.digits, k=3)
        ),
...
def generate_firm(id):
    ...
    firm_type = random.choice(
        ["bank", "investment bank", "asset manager", "wealth manager", "fund manager"]
    )
...
def generate_commitments(firms):
    ...
    for _ in range(300):  # Generate 300 commitments
        firm_id = random.choice(
            [2670, 2792, 332, 3611]
        )  # Randomly choose from specified firm_ids
...
data = {"firms": firms, "commitments": commitments}
